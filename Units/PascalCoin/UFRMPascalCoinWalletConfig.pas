...
Var df : Int64;
  ...
  i : Integer;
begin
  if udInternetServerPort.Position = udJSONRPCMinerServerPort.Position then raise Exception.Create(rsPCWalletCfgClkExcptSvrPort); // Server port and JSON-RPC Server miner port are equal!
  ...
  if TAccountComp.TxtToMoney(ebDefaultFee.Text,df) then begin
    ...
    raise Exception.Create(rsPCWalletCfgClkInvalFeeVal); // Invalid Fee value
  end;
  ...
  if rbGenerateANewPrivateKeyEachBlock.Checked then mpk := mpk_NewEachTime
    ...
    mpk := mpk_Selected;
    if cbPrivateKeyToMine.ItemIndex<0 then raise Exception.Create(rsPCWalletCfgMustSelPvtKey); // Must select a private key
    i := PtrInt(cbPrivateKeyToMine.Items.Objects[cbPrivateKeyToMine.ItemIndex]);
    if (i<0) Or (i>=FWalletKeys.Count) then raise Exception.Create(rsPCWalletCfgInvalidPvtKey); // Invalid private key
...
begin
  ...
  if Not FWalletKeys.IsValidPassword then begin
    s := '';
    Repeat
      if Not InputQuery(rsUpDPswClkWalltPswInpQueryC,rsUpDPswClkWalltPswInpQueryE,s) then exit; // Wallet Password - Insert Wallet Password
      FWalletKeys.WalletPassword := s;
      if Not FWalletKeys.IsValidPassword then Application.MessageBox(PChar(rsUpDPswClkInvalidPsw),PChar(Application.Title),MB_ICONERROR+MB_OK); // Invalid Password
    Until FWalletKeys.IsValidPassword;
  end;
  if FWalletKeys.IsValidPassword then begin
    s := ''; s2 := '';
    if Not InputQuery(rsUpDPswClkInpQueryChgPswC,rsUpDPswClkInpQueryChgPswE,s) then exit; //  Change password -  Type new password
    if trim(s)<>s then raise Exception.Create(rsUpDPswClkExpPswCantSpcChar); // Password cannot start or end with a space character
    if Not InputQuery(rsUpDPswClkInpQueryChgPswC,rsUpDPswClkInpQueryTypeAgain,s2) then exit; // Change password - Type new password again
    if s<>s2 then raise Exception.Create(rsUpDPswClkTwoPswDifferent); // Two passwords are different!

    FWalletKeys.WalletPassword := s;
    Application.MessageBox(PChar(Format(rsUpDPswClkPswChanged,s)), // Password changed!  Please note that your new password is "%s"  (If you lose this password, you will lose your Wallet forever !)
...
begin
  lblDefaultInternetServerPort.Caption := Format(rsPCWalletFormCreateSvrPort,[CT_NetServer_Port]); // (Default %d)
  ...
  UpdateWalletConfig;
  lblDefaultJSONRPCMinerServerPort.Caption := Format(rsPCWalletFormCreateSvrPort,[CT_JSONRPCMinerServer_Port]); // (Default %d)
end;
...
begin
  ...
  Except
    On E:Exception do begin
      TLog.NewLog(lterror,ClassName,Format(rsSetAppParamsException,+E.Message)); // Exception at SetAppParams: %s
    end;
  End;
...
begin
  if Assigned(FWalletKeys) then begin
    if FWalletKeys.IsValidPassword then begin
      if FWalletKeys.WalletPassword='' then begin
        bbUpdatePassword.Caption := rsUpDWltCfgWalletWithoutPsw; // Wallet without password, protect it!
      end else begin
        bbUpdatePassword.Caption := rsUpDWltCfgChangeWalletPsw; // Change Wallet password
      end;
    end else begin
        bbUpdatePassword.Caption := rsUpDWltCfgWalltWPswChangeIt; // Wallet with password, change it!
    end;
  ...
  end else bbUpdatePassword.Caption := rsPCWalletCfgbbUpDatePswC; // (Wallet password)
