...
begin
  ...
  if Not InputQuery(rsPLDecdrSavMthsClkInpQueryC,rsPLDecdrSavMthsClkInpQueryE,oph) then exit;  // Search operation by OpHash - Insert Operation Hash value (OpHash)
  DoFind(oph);
end;
...
begin
  ...
  lblPasswordsInfo.Caption := Format(rsPLDecMthdPubPLClkInpQueryC,[memoPasswords.Lines.Count]); // Possible passwords: %d
end;
...
begin
  ...
  try
    ...
    if (r='') then begin
      raise Exception.Create(rsPLDecDoFindValIsNHexaExcpt); //  Value is not an hexadecimal string
    end;
    pcops := TPCOperationsComp.Create(Nil);
    try
      If not TNode.Node.FindOperation(pcops,r,b,opbi) then begin
        raise Exception.Create(rsPLDecDoFindValIsNotValidOH); // Value is not a valid OpHash
      end;
      If not TPCOperation.OperationToOperationResume(b,pcops.Operation[opbi],pcops.Operation[opbi].SignerAccount,opr) then begin
        raise Exception.Create(rsPLDecDoFindExcptIntError); // Internal error 20161114-1
      end;
...
begin
  ...
  if PageControl.ActivePage=tsDecodeMethods then begin
    If not FSavedDecodeMethods then begin
      case Application.MessageBox(PChar(rsPLPgCtrlCghSaveNewDecMeths),PChar(Application.Title),MB_YESNOCANCEL+MB_ICONQUESTION) of // Save new decode methods?
        IDYES : Begin
          SaveMethods;
        End;
        IDCANCEL : begin
...
begin
  ...
  Try
    ...
    if Value.time>10000 then begin
      ...
      lblDateTime.Caption := rsPLDecSetOpReslblDateTimeC; // (Pending block)
    ...
    If (Value.SignerAccount>=0) And (Value.DestAccount>=0) then begin
      lblSenderCaption.Caption := rsPLDecSetOpReslblSenderSndC; // Sender:
      ...
      lblSenderCaption.Caption := rsPLDecSetOpReslblSenderAccC // Account:
  ...
  if Assigned(FWalletKeys) And Assigned(FAppParams) then begin
    ...
    if raw<>'' then begin
      ...
      if (cbMethodPublicPayload.Checked) and (TCrypto.IsHumanReadable(raw)) then begin
        ...
        lblDecodedMethod.Caption := rsPLDecTryDclblDMtNotEncrPLC; // Not encrypted payload
        ...
        lblDecodedMethod.Caption := Format(rsPLDecTryDclblDMtEncrWtEC_C,TAccountComp.GetECInfoTxt(WalletKey.PrivateKey.EC_OpenSSL_NID)); // Encrypted with EC %s
      end else if (cbUsingPasswords.Checked) And (UsePassword(raw,Decrypted,PasswordUsed)) then begin
        if cbShowAsHexadecimal.Checked then memoDecoded.Lines.Text := TCrypto.ToHexaString(Decrypted)
        else memoDecoded.Lines.Text := Decrypted;
        lblDecodedMethod.Caption := Format(rsPLDecTryDclblDMtEncrWtPsw,PasswordUsed); // Encrypted with pwd:"%s"
      end else begin
        memoDecoded.Lines.Text := rsPLDecTryDcMemoDcdTxtCNDecr; // CANNOT DECRYPT
        lblDecodedMethod.Caption := '';
        ok := false;
      end;
      if ok then begin
      ...
      memoDecoded.Lines.Text := rsPLDecTryDcMemoDcdTxtNPLoad; // (No payload)
