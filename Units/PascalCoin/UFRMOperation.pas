...
begin
  if Not Assigned(WalletKeys) then raise Exception.Create(rsNoWalletKeys);
  ...
  Try
    ...
    for iAcc := 0 to FSenderAccounts.Count - 1 do begin
      ...
      If Not UpdatePayload(account, errors) then
        raise Exception.Create(Format(rsErrorEncondingPayload,TAccountComp.AccountNumberToAccountTxtNumber(account.account),errors));
      i := WalletKeys.IndexOfAccountKey(account.accountInfo.accountKey);
      if i<0 then begin
        Raise Exception.Create(rsSndrAcnnPvtKeyNotFoundW);
      end;
      ...
      if PageControlOpType.ActivePage = tsTransaction then begin
        ...
        operationstxt := Format(rsOperationsTxtTransactTo,TAccountComp.AccountNumberToAccountTxtNumber(destAccount.account); // Transaction to %s
        {%endregion}
        ...
        inc(_totalfee,_fee);
        operationstxt := Format(rsOperationsTxtChangePvtKey,TAccountComp.GetECInfoTxt(_newOwnerPublicKey.EC_OpenSSL_NID); // Change private key to %s
        {%endregion}
        ...
        raise Exception.Create(rsNoOperationSelected);
      end;
    ...
    if (ops.OperationsCount=0) then raise Exception.Create(rsNoValidOperationToExecute);

    if (FSenderAccounts.Count>1) then begin
      if PageControlOpType.ActivePage = tsTransaction then auxs := Format(rsTotalAmountDestWillReceiv,TAccountComp.FormatMoney(_totalamount))
      else auxs:='';
      if Application.MessageBox(PChar(Format(rsExecSenderAccnOperations,Inttostr(FSenderAccounts.Count),operationstxt,auxs,TAccountComp.FormatMoney(_totalfee))),
        PChar(Application.Title),MB_YESNO+MB_ICONINFORMATION+MB_DEFBUTTON2)<>IdYes then exit;
    end else begin
      if Application.MessageBox(PChar(Format(rsExececuteThisOperation,operation_to_string)),
        PChar(Application.Title),MB_YESNO+MB_ICONINFORMATION+MB_DEFBUTTON2)<>IdYes then exit;
    end;
    i := FNode.AddOperations(nil,ops,Nil,errors);
    if (i=ops.OperationsCount) then begin
      Application.MessageBox(PChar(Format(rsSuccesfullyExecutedOpers,inttostr(i),operation_to_string)),PChar(Application.Title),MB_OK+MB_ICONINFORMATION);
      ModalResult := MrOk;
    end else if (i>0) then begin
      Application.MessageBox(PChar(Format(rsOneOrMoreOpsHasNotBeenExe,errors,inttostr(i))),PChar(Application.Title),MB_OK+MB_ICONWARNING); // One or more of your operations has not been executed:#10Errors:#1%s#10#10
      ModalResult := MrOk;
    end else begin
...
begin
  if FWalletKeys.IsValidPassword then begin
    ...
    s := '';
    Repeat
      if Not InputQuery(rsInpQueryWalletPassCaption,rsInpQueryWalletPassEnter,s) then exit; // Wallet password - Enter wallet password
...
begin
  ...
  Try
    ...
    lblAccountsCount.caption := Format(rsAccountsFormat,inttostr(senderAccounts.Count));
    balance := 0;
    if SenderAccounts.Count<=0 then begin
      ebSenderAccount.Text := '';
      ...
      for i := 0 to SenderAccounts.Count - 1 do begin
         ...
         if (accountstext<>'') then accountstext:= Format('; %s', accountstext);
         accountstext := Format(accountstext+'%s (%s)',TAccountComp.AccountNumberToAccountTxtNumber(acc.account),TAccountComp.FormatMoney(acc.balance));
      end;
...
begin
  ...
  if trim(ebFee.Text)<>'' then begin
    ...
    if not Result then errors := Format(rsInvalidFeeValue,ebFee.Text');
  end else begin
    Fee := 0;
    Result := true;
  end;
...
begin
  ...
  Try
    if SenderAccounts.Count<>1 then begin
      errors := rsUpDOpBuyAccnCantBuyMultiOp; // Cannot buy accounts with multioperations. Use only 1 account
      exit;
    end;
    If (Not TAccountComp.AccountTxtNumberToAccountNumber(ebAccountToBuy.Text,c)) then begin
      errors := Format(rsUpDOpBuyAccnInvalidAccnBuy,ebAccountToBuy.Text); // Invalid account to buy %s
      exit;
    end;
    If (c<0) Or (c>=FNode.Bank.AccountsCount) Or (c=SenderAccount.account) then begin
      errors := rsUpDOpBuyAccnInvalidAccnNum; // Invalid account number
      exit;
    end;
    ...
    If not TAccountComp.IsAccountForSale(AccountToBuy.accountInfo) then begin
      errors := Format(rsUpDOpBuyAccnIsNotForSale,TAccountComp.AccountNumberToAccountTxtNumber(c));
      exit;
    end;
    If Not TAccountComp.TxtToMoney(ebBuyAmount.Text,amount) then begin
      errors := rsUpDOpBuyAccnInvAmountValue; // Invalid amount value
      exit;
    end;
    if (AccountToBuy.accountInfo.price>amount) then begin
      errors := Format(rsUpDOpBuyAccnPrice,TAccountComp.FormatMoney(AccountToBuy.accountInfo.price)); // Account price %s
      exit;
    end;
    if (amount+DefaultFee > SenderAccount.balance) then begin
      errors := rsUpDOpBuyAccnInsufficientFd; // Insufficient funds
      exit;
    end;
    if cbBuyNewKey.ItemIndex<0 then begin
      errors := rsUpDOpBuyAccnMustSelNewPKey; // Must select a new private key;
      exit;
    end;
    i := PtrInt(cbBuyNewKey.Items.Objects[cbBuyNewKey.ItemIndex]);
    if (i<0) Or (i>=WalletKeys.Count) then raise Exception.Create(rsUpDOpBuyAccnInvalidSlcdKey); // Invalid selected key
    NewPublicKey := WalletKeys.Key[i].AccountKey;
    If (FNode.Bank.SafeBox.CurrentProtocol=CT_PROTOCOL_1) then begin
      errors := rsUpDOpBuyAccnOperNeedPr2Act; // This operation needs PROTOCOL 2 active
      exit;
    end;
  Finally
    Result := errors = '';
...
begin
  ...
  try
    if SenderAccounts.Count<>1 then begin
      errors := rsUpDChgInfoCantChgMultiOper; // Cannot change info with multioperations. Use only 1 account
      exit;
    end;
    if (TAccountComp.IsAccountLocked(TargetAccount.accountInfo,FNode.Bank.BlocksCount)) then begin
      errors := Format(rsUpDChgInfoAccnLockUntilBlc,TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account),
                                                    IntToStr(TargetAccount.accountInfo.locked_until_block)); // Account %s is locked until block %s
      exit;
    end;
    // Signer:
    If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSignerAccount.Text,auxC) then begin
      errors := rsUpDChgInfoInvaldSignerAccn; // Invalid signer account
      exit;
    end;
    if (auxC<0) Or (auxC >= FNode.Bank.AccountsCount) then begin
      errors := Format(rsUpDChgInfoSignerDNotExists,TAccountComp.AccountNumberToAccountTxtNumber(auxC)); // Signer account does not exists %s
      exit;
    end;
    ...
    if (TAccountComp.IsAccountLocked(SignerAccount.accountInfo,FNode.Bank.BlocksCount)) then begin
      errors := Format(rsUpDChgInfoSignerLockUntBlc,TAccountComp.AccountNumberToAccountTxtNumber(SignerAccount.account),
                                                    IntToStr(SignerAccount.accountInfo.locked_until_block)); // Signer account %s is locked until block %s
      exit;
    end;
    if (Not TAccountComp.EqualAccountKeys(SignerAccount.accountInfo.accountKey,TargetAccount.accountInfo.accountKey)) then begin
      errors := Format(rsUpDChgInfoSignerIsntOwerAc,TAccountComp.AccountNumberToAccountTxtNumber(SignerAccount.account),
                                                    TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account); // Signer account %s is not ower of account %s
      exit;
    end;
    If (FNode.Bank.SafeBox.CurrentProtocol=CT_PROTOCOL_1) then begin
      errors := rsUpDChgInfoThisOperPr2Actve; // This operation needs PROTOCOL 2 active
      exit;
    end;
    // New name and type
    ...
    If newName<>TargetAccount.name then begin
      ...
      If newName<>'' then begin
        if (Not TPCSafeBox.ValidAccountName(newName,errors)) then begin
          errors := Format(rsUpDChgInfoNewNameIsntvalid,newName,errors); // "%s" is not a valid name: %s
          Exit;
        end;
        i := (FNode.Bank.SafeBox.FindAccountByName(newName));
        if (i>=0) then begin
          errors := Format(rsUpDChgInfoNameIsUsedByAccn,newName,TAccountComp.AccountNumberToAccountTxtNumber(i)); // Name "%s" is used by account %s
          Exit;
        end;
      end;
    ...
    if (errCode>0) then begin
      errors := Format(rsUpDChgInfoInvalidType,ebChangeType.text); // Invalid type %s
      Exit;
    end;
    ...
    If (newName=TargetAccount.name) And (newType=TargetAccount.account_type) then begin
      errors := rsUpDChgInfoAccnNameTypeSame; // Account name and type are the same. Not changed
      Exit;
    end;
  finally
    Result := errors = '';
    if Not Result then begin
      ...
      lblChangeInfoErrors.Caption := Format(rsUpDChgInfolblChangeInfoErs,TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account)); // %s can be updated
    end;
  end;
end;
  ...
  try
    if rbChangeKeyWithAnother.Checked then begin
      if cbNewPrivateKey.ItemIndex<0 then begin
        errors := rsUpDOpChgKeyMustSelNwPvtKey; // Must select a new private key
        lblChangeKeyErrors.Caption := errors;
        exit;
      end;
      i := PtrInt(cbNewPrivateKey.Items.Objects[cbNewPrivateKey.ItemIndex]);
      if (i<0) Or (i>=WalletKeys.Count) then raise Exception.Create(rsUpDOpChgKeyInvalidSlcdKey); // Invalid selected key
      NewPublicKey := WalletKeys.Key[i].AccountKey;
      ...
      If Not TAccountComp.AccountKeyFromImport(ebNewPublicKey.Text,NewPublicKey,errors) then begin
        ...
        lblNewOwnerErrors.Caption := Format(rsUpDOpChgKeyNewKeyType,TAccountComp.GetECInfoTxt(NewPublicKey.EC_OpenSSL_NID)); // New key type: %s
        lblNewOwnerErrors.Font.Color := clGreen;
      end;
    end else begin
      errors := rsUpDOpChgKeySelectChgType; // Select a change type
      lblChangeKeyErrors.Caption := errors;
      exit;
    end;
    If FNode.Bank.SafeBox.CurrentProtocol>=1 then begin
      // Signer:
      If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSignerAccount.Text,auxC) then begin
        errors := rsUpDOpChgKeyInvaldSignrAccn; // Invalid signer account
        exit;
      end;
      if (auxC<0) Or (auxC >= FNode.Bank.AccountsCount) then begin
        errors := Format(rsUpDOpChgKeySignrAcDNtExist,TAccountComp.AccountNumberToAccountTxtNumber(auxC)); // Signer account does not exists %s
        exit;
      end;
      SignerAccount := FNode.Operations.SafeBoxTransaction.Account(auxC);
      if (TAccountComp.IsAccountLocked(SignerAccount.accountInfo,FNode.Bank.BlocksCount)) then begin
        errors := Format(rsUpDOpChgKeySignrAcLockUBlc,TAccountComp.AccountNumberToAccountTxtNumber(SignerAccount.account),
                                                      IntToStr(SignerAccount.accountInfo.locked_until_block)); // Signer account %s is locked until block %s
        exit;
      end;
      if (Not TAccountComp.EqualAccountKeys(SignerAccount.accountInfo.accountKey,TargetAccount.accountInfo.accountKey)) then begin
        errors := Format(rsUpDOpChgKeySignrAcIsNtOwer,TAccountComp.AccountNumberToAccountTxtNumber(SignerAccount.account),
                                                      TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account)); // Signer account %s is not ower of account %s
        exit;
      end;
    ...
    if (TAccountComp.EqualAccountKeys(TargetAccount.accountInfo.accountKey,NewPublicKey)) then begin
      errors := rsUpDOpChgKeyNPublKeySameKey; // New public key is the same public key
      lblChangeKeyErrors.Caption := errors;
      lblNewOwnerErrors.Caption := errors;
      exit;
    end;
...
begin
  ...
  try
    if Not TAccountComp.IsAccountForSale(TargetAccount.accountInfo) then begin
      errors := Format(rsUpDOpDeLstAccnIsNotForSale,TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account)); // Account %s is not for sale
      exit;
    end;
    if (TAccountComp.IsAccountLocked(TargetAccount.accountInfo,FNode.Bank.BlocksCount)) then begin
      errors := Format(rsUpDOpDeLstAccnLockUBlc,TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account),
                                                IntToStr(TargetAccount.accountInfo.locked_until_block)); // Account %s is locked until block %s
      exit;
    end;
    // Signer:
    If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSignerAccount.Text,auxC) then begin
      errors := rsUpDOpDeLstInvalidSignrAccn; // Invalid signer account
      exit;
    end;
    if (auxC<0) Or (auxC >= FNode.Bank.AccountsCount) then begin
      errors := Format(rsUpDOpDeLstSignrAccnDNExsts,TAccountComp.AccountNumberToAccountTxtNumber(auxC)); // Signer account does not exists %s
      exit;
    end;
    SignerAccount := FNode.Operations.SafeBoxTransaction.Account(auxC);
    if (TAccountComp.IsAccountLocked(SignerAccount.accountInfo,FNode.Bank.BlocksCount)) then begin
      errors := Format(rsUpDOpDeLstAccnLockUntilBlc,TAccountComp.AccountNumberToAccountTxtNumber(SignerAccount.account),
                                                    IntToStr(SignerAccount.accountInfo.locked_until_block)); // Signer account %s is locked until block %s
      exit;
    end;
    if (Not TAccountComp.EqualAccountKeys(SignerAccount.accountInfo.accountKey,TargetAccount.accountInfo.accountKey)) then begin
      errors := Format(rsUpDOpDeLstSignrAccnIsNOwer,TAccountComp.AccountNumberToAccountTxtNumber(SignerAccount.account),
                                                    TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account)); // Signer account %s is not ower of delisted account %s
      exit;
    end;
    If (FNode.Bank.SafeBox.CurrentProtocol=CT_PROTOCOL_1) then begin
      errors := rsUpDOpDeLstOperNeedsPr2Act; // This operation needs PROTOCOL 2 active
      exit;
    end;
  finally
    Result := errors = '';
    if Not Result then begin
      ...
      lblDelistErrors.Caption := Format(rsUpDOpDeLstCanBeDeListed,TAccountComp.AccountNumberToAccountTxtNumber(TargetAccount.account)); // %s can be delisted
    end;
  end;
end;
...
begin
  ...
  try
    ...
    if Not Assigned(WalletKeys) then begin
      errors := rsUpDOperOptNoWalletKeys; // No wallet keys
      lblGlobalErrors.Caption := errors;
      exit;
    end;
    if SenderAccounts.Count=0 then begin
      errors := rsUpDOperOptNoSenderAccount; // No sender account
      lblGlobalErrors.Caption := errors;
      exit;
    end else begin
      for iAcc := 0 to SenderAccounts.Count - 1 do begin
        ...
        if (iWallet<0) then begin
          errors := Format(rsUpDOperOptPrivateKeyOfAccn,TAccountComp.AccountNumberToAccountTxtNumber(sender_account.account)); // Private key of account %s not found in wallet
          lblGlobalErrors.Caption := errors;
          exit;
        end;
        ...
        if not assigned(wk.PrivateKey) then begin
          if wk.CryptedKey<>'' then begin
            errors := rsUpDOperOptWalletIsPswPrtcd; // Wallet is password protected. Need password
            bbPassword.Visible := true;
            bbPassword.Enabled := true;
          end else begin
            errors := Format(rsUpDOperOptOnlyPubKeyAccn,TAccountComp.AccountNumberToAccountTxtNumber(sender_account.account); // Only public key of account %s found in wallet. You cannot operate with this account
          end;
          lblGlobalErrors.Caption := errors;
          exit;
        end;
  ...
  if (PageControlOpType.ActivePage = tsTransaction) then begin
    ...
    errors := rsUpDOperOptMustSelOperation; // Must select an operation
  end;
  if (PageControlOpType.ActivePage=tsTransaction) then begin
    rbEncryptedWithOldEC.Caption := rsUpDOEncrypOldEncSenderKey; // Encrypted with sender public key
    rbEncryptedWithEC.Caption := rsUpDOEncrypOldEncDestPubKey; // Encrypted with destination public key
  end else if (PageControlOpType.ActivePage=tsChangePrivateKey) then begin
    rbEncryptedWithOldEC.Caption := rsUpDOEncryOldEncWtOldPubKey; // Encrypted with old public key
    rbEncryptedWithEC.Caption := rsUpDOEncryOldEncWtNewPubKey; // Encrypted with new public key
  end else if ((PageControlOpType.ActivePage=tsListForSale) Or (PageControlOpType.ActivePage=tsDelist)) then begin
    rbEncryptedWithOldEC.Caption := rsUpDOEncryOldEncWtTgtPubKey; // Encrypted with target public key
    rbEncryptedWithEC.Caption := rsUpDOEncryEncWtSignerPubKey; // Encrypted with signer public key
  end else if (PageControlOpType.ActivePage=tsBuyAccount) then begin
    rbEncryptedWithOldEC.Caption := rsUpDEncryEncWithBuyerPubKey; // Encrypted with buyer public key
    rbEncryptedWithEC.Caption := rsUpDEncryEncWithTgtPubKey; // Encrypted with target public key
  end;
  ...
  Try
    if (rbListAccountForPublicSale.Checked) Or (rbListAccountForPrivateSale.Checked) then begin
      ...
      if not TAccountComp.TxtToMoney(ebSalePrice.Text,salePrice) then begin
        errors := rsUpDOpLstSaleInvalidPrice; // Invalid price
        exit;
      end;
      // Signer:
      If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSignerAccount.Text,auxC) then begin
        errors := rsUpDOpLstSaleInvalSignrAccn; // Invalid signer account
        exit;
      end;
      if (auxC<0) Or (auxC >= FNode.Bank.AccountsCount) then begin
        errors := Format(rsUpDOpLstSaleSignrAcDNExsts,TAccountComp.AccountNumberToAccountTxtNumber(auxC)); // Signer account does not exists %s
        exit;
      end;
      SignerAccount := FNode.Operations.SafeBoxTransaction.Account(auxC);
      // Seller
      If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSellerAccount.Text,auxC) then begin
        errors := rsUpDOpLstSaleInvalSellerAcc; // Invalid seller account
        exit;
      end;
      if (auxC<0) Or (auxC >= FNode.Bank.AccountsCount) then begin
        errors := Format(rsUpDOpLstSaleSellerAcDNExts,TAccountComp.AccountNumberToAccountTxtNumber(auxC));
        exit;
      end;
      if (auxC=TargetAccount.account) then begin
        errors := rsUpDOpLstSaleSellerAcCantBS; // Seller account cannot be same account
        exit;
      end;
      ...
      if rbListAccountForPrivateSale.Checked then begin
        ...
        If Not TAccountComp.AccountKeyFromImport(ebSaleNewOwnerPublicKey.Text,NewOwnerPublicKey,errors) then begin
          errors := Format(rsUpDOpLstSalePublicKey,errors); // Public key: %s
          exit;
        end else begin
          lblListAccountErrors.Font.Color := clGreen;
          lblListAccountErrors.Caption := Format(rsUpDOpLstSaleNewKeyType,TAccountComp.GetECInfoTxt(NewOwnerPublicKey.EC_OpenSSL_NID)); // New key type: %s
        end;
        if TAccountComp.EqualAccountKeys(NewOwnerPublicKey,TargetAccount.accountInfo.accountKey) then begin
          errors := rsUpDOpLstSaleNewPKeyPvtSale; // New public key for private sale is the same public key
          Exit;
        end;
        LockedUntilBlock := StrToIntDef(ebSaleLockedUntilBlock.Text,0);
        if LockedUntilBlock=0 then begin
          errors := rsUpDOpLstSaleInsLockingBlck; // Insert locking block
          exit;
        end;
      end;
      If (FNode.Bank.SafeBox.CurrentProtocol=CT_PROTOCOL_1) then begin
        errors := rsUpDOpLstSaleThisOperNP2Act; // This operation needs PROTOCOL 2 active
        exit;
      end;
    end else begin
      lblSaleNewOwnerPublicKey.Enabled := false;
      ...
      ebSaleLockedUntilBlock.Enabled := false;
      lblSaleLockedUntilBlock.Enabled := false;
      errors := rsUpDOpLstSaleSelectSaleType; // Select a sale type
      exit;
    end;
  Finally
    Result := errors='';
...
begin
  ...
  if not (TAccountComp.AccountTxtNumberToAccountNumber(ebDestAccount.Text,c)) then begin
    errors := Format(rsUpDOpTrnsInvalidDestAccn,ebDestAccount.Text); // Invalid dest. account (%s)
    lblTransactionErrors.Caption := errors;
    exit;
  end;
  if (c<0) Or (c>=TNode.Node.Bank.AccountsCount) then begin
    errors := Format(rsUpDOpTrnsInvalidDestAccn,TAccountComp.AccountNumberToAccountTxtNumber(c)); // Invalid dest. account (%s)
    lblTransactionErrors.Caption := errors;
    exit;
  end;
  ...
  if SenderAccounts.Count=1 then begin
    if not TAccountComp.TxtToMoney(ebAmount.Text,amount) then begin
      errors := Format(rsUpDOpTrnsInvalidAmount,ebAmount.Text); // Invalid amount (%s)
      lblTransactionErrors.Caption := errors;
      exit;
    end;
  end else amount := 0; // ALL BALANCE
  if DestAccount.account=SenderAccount.account then begin
    errors := rsUpDOpTrnsSndrAndDestAcSame; // Sender and dest account are the same
    lblTransactionErrors.Caption := errors;
    exit;
  end;
  if (SenderAccounts.Count=1) then begin
    if (SenderAccount.balance<(amount+FDefaultFee)) then begin
       errors := rsUpDOpTrnsInsufficientFunds; // Insufficient funds
       lblTransactionErrors.Caption := errors;
       exit;
    end;
  end;
...
begin
  ...
  payload_encrypted := '';
  FEncodedPayload := '';
  errors := rsUpDPayLoadUnknowError; // Unknown error
  ...
  try
    ...
    if (rbEncryptedWithOldEC.Checked) then begin
      // Use sender
      errors := rsUpDPayloadErrorEncrypting; // Error encrypting
      ...
      if (PageControlOpType.ActivePage=tsTransaction) or (PageControlOpType.ActivePage=tsListForSale) or (PageControlOpType.ActivePage=tsDelist)
        ...
        If (PageControlOpType.ActivePage=tsTransaction) then begin
          If Not TAccountComp.AccountTxtNumberToAccountNumber(ebDestAccount.Text,dest_account_number) then begin
            errors := rsUpDPayLoadInvalidDestAcNum; // Invalid dest account number
            exit;
          end;
        end else if (PageControlOpType.ActivePage=tsListForSale) then begin
          If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSignerAccount.Text,dest_account_number) then begin
            errors := rsUpDPayLoadInvalidSngrAcNum; // Invalid signer account number
            exit;
          end;
        end else if (PageControlOpType.ActivePage=tsDelist) then begin
          If Not TAccountComp.AccountTxtNumberToAccountNumber(ebSignerAccount.Text,dest_account_number) then begin
          ...
          If Not TAccountComp.AccountTxtNumberToAccountNumber(ebAccountToBuy.Text,dest_account_number) then begin
            errors := rsUpDPayLoadInvalidAccBuyNum; // Invalid account to buy number
            exit;
          end;
        end else begin
          errors := rsUpDPayLoadErrorDev20170512; // ERROR DEV 20170512-1
          exit;
        end;
        if (dest_account_number<0) or (dest_account_number>=FNode.Bank.AccountsCount) then begin
          errors := Format(rsUpDPayLoadInvalidPLEnAcNum,TAccountComp.AccountNumberToAccountTxtNumber(dest_account_number)); // Invalid payload encrypted account number: %s
          exit;
        end;
        ...
        if (rbChangeKeyWithAnother.Checked) then begin
          ...
          if (cbNewPrivateKey.ItemIndex>=0) then begin
            ...
            errors := rsUpDPayLoadMustSelPrivatKey; // Must select a private key
            exit;
          end;
        end else if (rbChangeKeyTransferAccountToNewOwner.Checked) then begin
          If Not TAccountComp.AccountKeyFromImport(ebNewPublicKey.Text,public_key,errors) then begin
            errors := Format(rsUpDPayLoadPublicKeyError,errors);
            exit;
          end;
        end else begin
          errors := rsUpDPayLoadMustSelChngeType; // Must select change type
          exit;
        end;
        if public_key.EC_OpenSSL_NID<>CT_Account_NUL.accountInfo.accountKey.EC_OpenSSL_NID then begin
          ...
          valid := false;
          errors := rsUpDPayLoadSelPvtKeyIsntVal; // Selected private key is not valid to encode
          exit;
        end;
      end else begin
        errors := rsUpDPayLoadOperDNAllwKindPL; // This operation does not allow this kind of payload
      end;
      ...
      errors := rsUpDPayLoadMustSelEncrOptPL; // Must select an encryption option for payload
    end;
  finally
    if valid then begin
      if length(payload_encrypted)>CT_MaxPayloadSize then begin
        valid := false;
        errors := Format(rsUpDPayLoadPLSizeIsBigger,inttostr(CT_MaxPayloadSize),Inttostr(length(payload_encrypted))); // Payload size is bigger than %s (%s)
      end;
    end;
    if valid then begin
