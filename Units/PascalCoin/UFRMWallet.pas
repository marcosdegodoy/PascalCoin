...
Uses UFolderHelper, UOpenSSL, UOpenSSLdef, UConst, UTime, UFileStorage, UStringsResources,
...
begin
  ...
  try
    // Check OpenSSL dll
    if Not LoadSSLCrypt then raise Exception.Create(Format(rsCannotLoadSSL, [SSL_C_LIB+#10]));
    TCrypto.InitCrypto;
    // Read Wallet
    Try
    ...
    Except
      On E:Exception do begin
        E.Message := rsCannotOpenWallet+#10+#10+E.Message;
        Raise;
      end;
    End;
  ...
  Except
    On E:Exception do begin
      E.Message := Format(rsWalletActivateError,E.Message); // An error occurred during initialization. Application cannot continue: #1#10%s#10#10Application will close...
...
begin
  ...
  if (i<0) Or (i>=FWalletKeys.Count) then raise Exception.Create(rsMustSelectAKey);
  name := FWalletKeys.Key[i].Name;
  if InputQuery(rsbbChKeyNameClkInpQueryC,rsbbChKeyNameClkInpQueryE,name) then begin // Change Key name - Input new name
...
begin
  CheckIsReady;
  if FSelectedAccountsGrid.AccountsCount<=0 then raise Exception.Create(rsSelectAtLeastOneAcn);
...
begin
  CheckIsReady;
  if (lbNetConnections.SelCount<=0) Or (lbNetConnections.ItemIndex<0) then raise Exception.Create(rsSelectAtLeastOneCnt);
  ...
  if trim(m)='' then raise Exception.Create(rsNoMessage);

  if Application.MessageBox(PChaR(Format('rsSendMessageNodes', inttostr(n), m]),PChar(Application.Title),MB_ICONQUESTION+MB_YESNO+MB_DEFBUTTON1)<>IdYes then exit;
  them := m;
  if n>1 then begin
    for i := 0 to lbNetConnections.Items.Count - 1 do begin
      if lbNetConnections.Selected[i] then begin
        ...
        if TNetData.NetData.ConnectionExistsAndActive(nc) then begin
          ...
          memoMessages.Lines.Add(DateTimeToStr(now)+ rsSentTo +nc.ClientRemoteAddr+' > '+m);
        end;
      end;
    end;
  end else begin
  ...
  Application.MessageBox(PChaR(Format(rsSentToNodes, inttostr(n), m)),PChar(Application.Title),MB_ICONINFORMATION+MB_OK);
end;
...
begin
  ...
  if Not FNode.IsReady(isready) then begin
    Raise Exception.Create(Format(rsCantDoOperationNow, isready));
  end;
end;

procedure TFRMWallet.CM_NetConnectionUpdated(var Msg: TMessage);
Const CT_BooleanToString : Array[Boolean] of String = (rsBooleanStrFalse,rsBooleanStrTrue);
Var i : integer;
 NC : TNetConnection;
 l : TList;
      ...
      Try
        for i := 0 to l.Count - 1 do begin
          ...
          If NC.Client.BytesReceived>0 then begin
            sClientApp := Format(rsBytesRcvdOk, IntToStr(NC.NetProtocolVersion.protocol_version, IntToStr(NC.NetProtocolVersion.protocol_available), NC.ClientAppVersion);
          end else begin
            sClientApp := rsBytesRcvdNoData;
          end;
          ...
          if NC.Connected then begin
            if NC.Client.LastCommunicationTime>1000 then begin
              ...
              if (hh=0) and (nn=0) And (ss<10) then begin
                sLastConnTime := rsLastCommLess10sec;
              end else begin
                sLastConnTime := Format(rsLastCommTime,[(hh*60)+nn,ss]);
              end;
            end else begin
              sLastConnTime := '';
            end;
            if NC is TNetServerClient then begin
              sNSC.Add(Format(rsClientIPSentReceived,
              ...
              if NC.IsMyselfServer then sNSC.Add(Format(rsMySelfIPSentReceived,
                ...
                sRS.Add(Format(rsRemoteSvrSentReceived,
                [NC.ClientRemoteAddr,NC.RemoteOperationBlock.block,NC.Client.BytesSent,NC.Client.BytesReceived,sClientApp,NC.TimestampDiff,DateTimeElapsedTime(NC.CreatedTime),sLastConnTime]));
              end;
            end;
          end else begin
            if NC is TNetServerClient then begin
              sDisc.Add(Format(rsDisconnectedClient,[NC.ClientRemoteAddr,sClientApp]));
            end else if NC.IsMyselfServer then begin
              sDisc.Add(Format(rsDisconnectedMySelf,[NC.ClientRemoteAddr,sClientApp]));
            end else begin
              sDisc.Add(Format(rsDisconnectedRemoteSvr,[NC.ClientRemoteAddr,CT_BooleanToString[NC.Connected],sClientApp]));
            end;
          end;
        end;
        strings.Clear;
        strings.Add(Format(rsConnectionsUptade,[DateTimeToStr(now),sNSC.Count,sRS.Count,TNetData.NetData.NetStatistics.ServersConnectionsWithResponse]));
        strings.AddStrings(sRS);
        strings.AddStrings(sNSC);
        if sDisc.Count>0 then begin
          strings.Add('');
          strings.Add(Format(rsDisconnectedConns,Inttostr(sDisc.Count)));
...
begin
  ...
  Strings.Add(Format(rsAccnInfoNumberType,[TAccountComp.AccountNumberToAccountTxtNumber(AccountNumber),s,account.account_type]));
  Strings.Add('');
  Strings.Add(Format(rsAccnInfoCurrBalance,[TAccountComp.FormatMoney(account.balance)]));
  Strings.Add('');
  Strings.Add(Format(rsAccnInfoUpdatedBlock,[account.updated_block,FNode.Bank.BlocksCount-account.updated_block]));
  Strings.Add(Format(rsAccnInfoPublicKey,[TAccountComp.GetECInfoTxt(account.accountInfo.accountKey.EC_OpenSSL_NID)]));
  Strings.Add(Format(rsAccnInfoBase58PubKey,[TAccountComp.AccountPublicKeyExport(account.accountInfo.accountKey)]));
  if TAccountComp.IsAccountForSale(account.accountInfo) then begin
    Strings.Add('');
    Strings.Add(rsAccnInfoAccnForSale);
    Strings.Add(Format(rsAccnInfoPrice,[TAccountComp.FormatMoney(account.accountInfo.price)]));
    Strings.Add(Format(rsAccnInfoSellerAccount,[TAccountComp.AccountNumberToAccountTxtNumber(account.accountInfo.account_to_pay)]));
    if TAccountComp.IsAccountForSaleAcceptingTransactions(account.accountInfo) then begin
      Strings.Add('');
      Strings.Add(rsAccnInfoPrivateSale);
      Strings.Add(Format(rsAccnInfoNewB58PubKey,[TAccountComp.AccountPublicKeyExport(account.accountInfo.new_publicKey)]));
      Strings.Add('');
      if TAccountComp.IsAccountLocked(account.accountInfo,FNode.Bank.BlocksCount) then begin
        Strings.Add(Format(rsAccnInfoSecPurchase,
        ...
        Strings.Add(Format(rsAccnInfoUnSecPurchase,
...
begin
  ...
  If OperationResume.Block<FNode.Bank.BlocksCount then
    if (OperationResume.NOpInsideBlock>=0) then begin
      Strings.Add(Format(rsOperInfoBlockNOpInside,[OperationResume.Block,OperationResume.NOpInsideBlock]))
    end else begin
      Strings.Add(Format(rsOperInfoBlock,[OperationResume.Block]))
    end
  else Strings.Add(rsOperInfoPendingOper);
  Strings.Add(Format('%s',[OperationResume.OperationTxt]));
  Strings.Add(Format(rsOperInfoOpTypeSubType,[OperationResume.OpType,OperationResume.OpSubtype]));
  Strings.Add(Format(rsOperInfoOperationHash,[TCrypto.ToHexaString(OperationResume.OperationHash)]));
  If (OperationResume.OperationHash_OLD<>'') then begin
    Strings.Add(Format(rsOperInfoOldOperHash,[TCrypto.ToHexaString(OperationResume.OperationHash_OLD)]));
  end;
  if (OperationResume.OriginalPayload<>'') then begin
    Strings.Add(Format(rsOperInfoPayloadLength,[length(OperationResume.OriginalPayload)]));
    If OperationResume.PrintablePayload<>'' then begin
      Strings.Add(Format(rsOperInfoPayloadHuman,[OperationResume.PrintablePayload]));
    end;
    Strings.Add(Format(rsOpenInfoPayloadHexa,[TCrypto.ToHexaString(OperationResume.OriginalPayload)]));
  end;
  If OperationResume.Balance>=0 then begin
    Strings.Add(Format(rsOperInfoFinalBalance,[TAccountComp.FormatMoney(OperationResume.Balance)]));
  end;
end;
...
begin
  ...
  FLog.SaveTypes := [];
  If Not ForceDirectories(TFolderHelper.GetPascalCoinDataFolder) then raise Exception.Create(Format(rsCantCreateDir, TFolderHelper.GetPascalCoinDataFolder));
  ...
  TrayIcon.BalloonTitle := rsTrayIconBalloonTitle;
  TrayIcon.BalloonHint := rsTrayIconBalloonHint;
  TrayIcon.BalloonFlags := bfInfo;
  MinersBlocksFound := 0;
  lblBuild.Caption := Format(rslblBuild,CT_ClientAppVersion);
  {$IFDEF TESTNET}
...
Var i : Integer;
  step : String;
begin
  TLog.NewLog(ltinfo,Classname,rsDestroyingFormStart);
  Try
  ...
  step := rsDestroyNodeNotifyEvnts;
  FreeAndNil(FNodeNotifyEvents);
  //
  step := rsAssigningNilToTNetData;
  TNetData.NetData.OnReceivedHelloMessage := Nil;
  TNetData.NetData.OnStatisticsChanged := Nil;

  step := rsDestroyingGridOpers;
  FreeAndNil(FOperationsAccountGrid);
  FreeAndNil(FOperationsExplorerGrid);
  FreeAndNil(FBlockChainGrid);

  step := rsOrderedAccountsKeyList;
  FreeAndNil(FOrderedAccountsKeyList);

  step := rsDesactivatingNode;
  TNode.Node.NetServer.Active := false;
  FNode := Nil;

  TNetData.NetData.Free;

  step := rsProcessingMessages1;
  Application.ProcessMessages;

  step := rsDestroyingNode;
  TNode.Node.Free;

  step := rsDestroyingWallet;
  FreeAndNil(FWalletKeys);
  step := rsProcessingMessages2;
  Application.ProcessMessages;
  step := rsDestroyingStringsList;
  Except
    On E:Exception do begin
      TLog.NewLog(lterror,Classname,Format(rsErrDestroyingFormStep,step,E.ClassName,E.Message));
    end;
  End;
  TLog.NewLog(ltinfo,Classname,rsDestroyingFormEnd);
  FreeAndNil(FLog);
  Sleep(100);
end;
...
begin
  ...
  if i<0 then begin
    ...
    try
      ...
      FWalletKeys.AddPrivateKey(Format(rsNewForMiner,DateTimeToStr(Now)), PK);
...
begin
  ...
  If FAppParams.FindParam(CT_PARAM_MinerName)=Nil then begin
    ...
    FAppParams.ParamByName[CT_PARAM_MinerName].SetAsString(Format(rsNewNodeIntNameFVersion,DateTimeToStr(Now),
                                                                  fvi.InternalName, fvi.FileVersion));
  end;
...
begin
  ...
  try
    ...
    if PageControl.ActivePage=tsOperations then begin
      ...
      if accn<0 then raise Exception.Create(rsSelectAnAccount);
      FillAccountInformation(strings,accn);
      title := Format(rsAccountNumber, TAccountComp.AccountNumberToAccountTxtNumber(accn));
    ...
    If (opr.valid) then begin
      ...
      else title := rsOperationInfo;
      strings.Add(rsOperationInfo+':');
      FillOperationInformation(strings,opr);
    end else if accn<0 then Raise Exception.Create(rsAccnNoInfoAvailable);
...
begin
  ...
  else raise Exception.Create(Format(rsIndexAccnHighBalance,TAccountComp.AccountNumberToAccountTxtNumber(start.account),TAccountComp.FormatMoney(start.balance)));
end;
...
begin
  oph := '';
  if Not InputQuery(rsFindOpHashClkInpQueryC,rsFindOpHashClkInpQueryE,oph) then exit;
  //
...
begin
  ...
  else raise Exception.Create(Format(rsPrevAccnHighBalanceClk,TAccountComp.AccountNumberToAccountTxtNumber(start.account),TAccountComp.FormatMoney(start.balance)));
end;
...
begin
  ...
  Try
    ...
    try
      If FAccountsGrid.SelectedAccounts(l)<1 then raise Exception.Create(rsNewOperClkNoRowSelect);
      SenderAccounts.CopyFrom(l);
    finally
      l.Free;
    end;
...
Const CT_TRUE_FALSE : Array[Boolean] Of AnsiString = (UpperCase(rsBooleanStrFalse),UpperCase(rsBooleanStrTrue));
Var i,j,n : integer;
...
begin
  ...
  try
    ...
    Try
      strings.Clear;
      strings.Add(Format(rsOnNetBlackListUpdated,DateTimeToStr(now),IntToHex(TThread.CurrentThread.ThreadID,8)));
      j := 0; n:=0;
      for i := 0 to l.Count - 1 do begin
        P := l[i];
        if (P^.is_blacklisted) then begin
          ...
          if Not P^.its_myself then begin
            inc(j);
            strings.Add(Format(rsOnNetBlkListIPLastConn,
              [
               P^.ip,P^.port,
      ...
      Strings.Add(Format(rsOnNetBlackListTotalIPs,[j,n]));
    Finally
      strings.EndUpdate;
    End;
  finally
...
begin
  ...
  try
    ...
    Try
      strings.Clear;
      strings.Add(Format(rsOnNetNodeSeverUpdated,DateTimeToStr(now),inttostr(l.Count)));
      for i := 0 to l.Count - 1 do begin
        P := l[i];
        if Not (P^.is_blacklisted) then begin
          s := Format(rsNetDataServerIP,[P^.ip,P^.port]);
          if Assigned(P.netConnection) then begin
            If P.last_connection>0 then  s := s+ rsNetDataActive
            else s := s + rsNetDataTryingToConnect;
          end;
          if P.its_myself then begin
            s := s + Format(rsNetDataNotValid,P.BlackListText);
          end;
          if P.last_connection>0 then begin
            s := s + Format(rsNetDataLastConnection,DateTimeToStr(UnivDateTime2LocalDateTime( UnixToUnivDateTime(P^.last_connection))));
          end;
          if P.last_connection_by_server>0 then begin
            s := s + Format(rsNetDataLastSeverConn,DateTimeToStr(UnivDateTime2LocalDateTime( UnixToUnivDateTime(P^.last_connection_by_server))));
          end;
          if (P.last_attempt_to_connect>0) then begin
            s := s + Format(rsNetDataLastAttpConnect,DateTimeToStr(P^.last_attempt_to_connect));
          end;
          if (P.total_failed_attemps_to_connect>0) then begin
            s := s + Format(rsNetDataAttempts,inttostr(P^.total_failed_attemps_to_connect));
          end;
...
begin
  ...
  if Assigned(FNode) then begin
    If FNode.NetServer.Active then begin
      StatusBar.Panels[0].Text := Format(rsOnNetStatChgActivPort,Inttostr(FNode.NetServer.Port));
    end else StatusBar.Panels[0].Text := rsOnNetStatChgSvrStopped;
    NS := TNetData.NetData.NetStatistics;
    StatusBar.Panels[1].Text := Format(rsOnNetStatChgConnClient,
...
begin
  ...
  Except
    On E:Exception do begin
      E.Message := Format(rsOnNewAccountError,E.Message);
      Raise;
    end;
  end;
end;
...
begin
  ...
  if ThreadID=MainThreadID then s := rsOnNewLogMessageMain else s:= rsOnNewLogMessageTID;
  ...
  memoLogs.Lines.Add(formatDateTime(rsOnNewLogFormatDateTime,Time)+s+IntToHex(ThreadID,8)+' ['+CT_LogType[Logtype]+'] <'+sender+'> '+logtext);
  //
end;
...
begin
  ...
  if Assigned(NetConnection) then begin
    s := Format(rsOnNodeMsgEvMsgRcvdFrom,DateTimeToStr(now),NetConnection.ClientRemoteAddr;
    memoMessages.Lines.Add(Format(rsOnNodeMsgEvRcvdLength,DateTimeToStr(now),NetConnection.ClientRemoteAddr,inttostr(Length(MessageData))));
    memoMessages.Lines.Add(Format(rsOnNodeMsgEvReceived,MessageData));
    if FAppParams.ParamByName[CT_PARAM_ShowModalMessages].GetAsBoolean(false) then begin
      s := Format(rsOnNodeMsgEvDateTime,DateTimeToStr(now),NetConnection.ClientRemoteAddr,inttostr(length(MessageData)));
      if TCrypto.IsHumanReadable(MessageData) then begin
         s := s + MessageData;
      end else begin
         s := s + Format(rsOnNodeMsgEvValueHexa,TCrypto.ToHexaString(MessageData));
      end;
      Application.MessageBox(PChar(s),PChar(Application.Title),MB_ICONINFORMATION+MB_OK);
    end;
  end else begin
    memoMessages.Lines.Add(Format(rsOnNodeMsgEvInternalMsg,MessageData));
  end;
  if FMessagesUnreadCount>1 then lblReceivedMessages.Caption := Format(rsOnNodeMsgEvReceivedMsg,[FMessagesUnreadCount])
  else lblReceivedMessages.Caption := rsOnNodeMsgEvRcvdOneMsg;
  ...
  Try
    ...
    Try
      for i := 0 to lsource.Count-1 do begin
        if FWalletKeys.IndexOfAccountKey(FNode.Bank.SafeBox.Account(lsource.Get(i)).accountInfo.accountKey)<0 then raise Exception.Create(Format(rssbSelectedAccnAddAllClk,[lsource.Get(i)]));
...
begin
  ...
  if (an<0) then raise Exception.Create(rssbSelectedAccnAddClkNo); // No account selected
  if FWalletKeys.IndexOfAccountKey(FNode.Bank.SafeBox.Account(an).accountInfo.accountkey)<0 then
    raise Exception.Create(Format(rssbSelectedAccnAddClkMsg,[TAccountComp.AccountNumberToAccountTxtNumber(an)])); // You cannot add %s account because private key not found in your wallet.'#10+#10+'You''re not the owner!
  // Add
  l := FSelectedAccountsGrid.LockAccountsList;
...
begin
  ...
  Except
    On E:Exception do begin
      E.Message := Format(rsTimerUpdateStatusTimer,E.ClassName,E.Message);
        ...
        if NC.Connected then begin
          if NC is TNetServerClient then begin
            if Not NC.IsMyselfServer then begin
              lbNetConnections.Items.AddObject(Format(rsNetDataNetConnClientIP,[NC.ClientRemoteAddr]),NC);
            end;
          end else begin
            if Not NC.IsMyselfServer then begin
              lbNetConnections.Items.AddObject(Format(rsNetDataNetConnServerIP,[NC.ClientRemoteAddr]),NC);
            end;
          end;
        end;
      end;
...
//  hr := 0;
  if Assigned(FNode) then begin
    if FNode.Bank.BlocksCount>0 then begin
      lblCurrentBlock.Caption := Format('%s (0..%s)',Inttostr(FNode.Bank.BlocksCount),Inttostr(FNode.Bank.BlocksCount-1));
    end else lblCurrentBlock.Caption :=  rsNone; // '(none)'
    ...
    lblTimeAverage.Caption := Format(rsUpdateBlockChnStateLast,Inttostr(CT_CalcNewTargetBlocksAverage),
                                                               FormatFloat('0.0',favg),
                                                               Inttostr(CT_NewLineSecondsAvg),
                                                               FormatFloat('0.00%',f*100));
    if favg>=CT_NewLineSecondsAvg then begin
      lblTimeAverage.Font.Color := clNavy;
    end else begin
      lblTimeAverage.Font.Color := clOlive;
    end;
    lblTimeAverageAux.Caption := Format(rsUpdateBlockChnStateSecs,[
    ...
    isMining := false;
    lblCurrentBlock.Caption := rslblCurrentBlockCaption;
    lblCurrentAccounts.Caption := rslblCurrentAccountsCaption;
    lblCurrentBlockTime.Caption := rslblCurrBlockTimeCaption;
    lblOperationsPending.Caption := rslblOpersPendingCaption;
    lblCurrentDifficulty.Caption := rslblCurrDifficCaption;
    lblTimeAverage.Caption := rslblTimeAverageCaption;
    lblTimeAverageAux.Caption := rslblTimeAverageAxCaption; // **  All labels = '' **
  end;
  if (Assigned(FPoolMiningServer)) And (FPoolMiningServer.Active) then begin
    If FPoolMiningServer.ClientsCount>0 then begin
      lblMinersClients.Caption := Format(rslblMnrsClientsCaptionCn,IntToStr(FPoolMiningServer.ClientsCount)); // connected JSON-RPC clients
      lblMinersClients.Font.Color := clNavy;
    end else begin
      lblMinersClients.Caption := rslblMnrsClientsCaptionNJ; // No JSON-RPC clients
      lblMinersClients.Font.Color := clDkGray;
    end;
    MinersBlocksFound := FPoolMiningServer.ClientsWins;
  end else begin
    MinersBlocksFound := 0;
    lblMinersClients.Caption := rslblMnrsClientsCaptionNA; // JSON-RPC server not active
...
begin
  ...
  if FAppParams.ParamByName[CT_PARAM_SaveLogFiles].GetAsBoolean(false) then begin
    ...
    FLog.FileName := Format(rsFLogFileName,TFolderHelper.GetPascalCoinDataFolder,PathDelim); // %s%sPascalCointWallet.log
  end else begin
    FLog.SaveTypes := [];
    FLog.FileName := '';
  end;
  if Assigned(FNode) then begin
    ...
    FNode.NodeLogFilename := Format(rsFNodeNodeLogFileName,TFolderHelper.GetPascalCoinDataFolder,PathDelim); // %s%sblocks.log
  end;
...
begin
  ...
  if Assigned(FNode) then begin
    if FNode.IsBlockChainValid(errors) then begin
      StatusBar.Panels[2].Text := Format(rsUpDConnStatLastAccnTime,
       [FormatDateTime(rsUpDConnFormatDateTime,UnivDateTime2LocalDateTime(UnixToUnivDateTime( FNode.Bank.LastOperationBlock.timestamp )))]);
    end else begin
      StatusBar.Panels[2].Text := Format(rsUpDConnNoBlockChain,errors);
    end;
  end else begin
...
begin
  If Not Assigned(FNode) then begin
    ...
    lblNodeStatus.Caption := rslblNodeStatCaptionInit; // Initializing...
  end else begin
    If FNode.IsReady(status) then begin
      if TNetData.NetData.NetStatistics.ActiveConnections>0 then begin
        ...
        if TNetData.NetData.IsDiscoveringServers then begin
          lblNodeStatus.Caption := rslblNodeStatCaptionDSvrs; // Discovering servers
        end else if TNetData.NetData.IsGettingNewBlockChainFromClient then begin
          lblNodeStatus.Caption := rslblNodeStatCaptionObNBC; // Obtaining new blockchain
        end else begin
          lblNodeStatus.Caption := rslblNodeStatCaptionRunng; // Running
        end;
      end else begin
        lblNodeStatus.Font.Color := clRed;
        lblNodeStatus.Caption := rslblNodeStatCaptionAlone; // Alone in the world...
      end;
    end else begin
      lblNodeStatus.Font.Color := clRed;
  ...
  If Assigned(FBackgroundPanel) then begin
    ...
    FBackgroundPanel.Caption:= Format(rsFBackgroundPanelCaption,lblNodeStatus.Caption); // Please wait until finished: %s
  end;
end;
  ...
  Try
    ...
    For i:=0 to FWalletKeys.Count-1 do begin
      ...
      if (wk.Name='') then begin
        s := Format('Sha256=%s',TCrypto.ToHexaString( TCrypto.DoSha256( TAccountComp.AccountKey2RawString(wk.AccountKey) ) ));
      end else begin
        s := wk.Name;
      end;
    ...
    cbMyPrivateKeys.Items.InsertObject(0,rsUpDPKeyscbMyPrivateKeys,TObject(-1)); // (All my private keys)
  Finally
