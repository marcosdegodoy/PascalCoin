...
uses
  UNetProtocol, UNode, UConst, UStringsResources;
...
begin
  ...
  if (length(nsarr)=0) And (cbTryOnlyWithThisServers.Checked) then begin
    raise Exception.Create(rsNoValidIPinList);
  end;
  // Encode
  ...
  if Assigned(FAppParams) then begin
    ...
    if cbTryOnlyWithThisServers.Checked then Begin
      ...
      Application.MessageBox(PChar(rsbbOkClickRestartMsg),PChar(Application.Title),MB_OK);
    end else begin
...
begin
  ...
  if cbTryOnlyWithThisServers.Checked then begin
    Application.MessageBox(PChar(Format(rscbTryOnlyWSrvsClkAlert,cbTryOnlyWithThisServers.Caption)) // 'ALERT: If "%s" is checked and no valid server found, you will be alone!
...
begin
  ...
  for i := low(nsarr) to high(nsarr) do begin
    aux := nsarr[i].ip;
    if nsarr[i].port>0 then aux := aux + Format(':%s',InttoStr(nsarr[i].port)); //
