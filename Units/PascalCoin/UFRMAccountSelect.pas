...
Uses strutils, UStringsResources;
...
begin
  ...
  If cbAccountsBalance.Checked then begin
    If not TAccountComp.TxtToMoney(ebMinBalance.Text,searchValues.minBal) then Raise Exception.Create(rsAccnSelelctInvMinBlncExcp); // Invalid Min Balance
    ebMinBalance.Text:=TAccountComp.FormatMoney(searchValues.minBal);
    If Trim(ebMaxBalance.Text)='' then begin
      ebMaxBalance.Text:='';
      searchValues.maxBal:=-1;
    end else begin
      If not TAccountComp.TxtToMoney(ebMaxBalance.Text,searchValues.maxBal) then Raise Exception.Create(rsAccnSelelctInvMaxBlncExcp); // Invalid Max Balance
...
begin
  ...
  If not Assigned(FWalletKeys) then begin
    ...
    try
      ...
      cbMyPrivateKeys.items.InsertObject(0,rsAccountSelectAll,TObject(-1)); // (All)
    finally
